...
$expenseQuery = $connection->prepare("SELECT SUM(amount) AS total_expense FROM expense WHERE MONTH(expense_date) = ? AND YEAR(expense_date) = ?");
    ...
    <ul>
        <li>
            ...
            <ul class="submenu-items">
                <li><a href="staff_analytics.php" class="active"><span class="las la-chart-line"></span> Analytics</a></li>
...
<div class="budget-meter">
    ...
    <p>Total Budget: <?= number_format($totalBudget, 2); ?></p> <!-- Display total budget -->
    <div class="meter">
        <div class="fill" style="width: <?= $budgetUsage; ?>%">
            <span><?= number_format($totalExpense, 2); ?></span> <!-- Display expense amount inside the bar -->
        </div>
    </div>
    ...
    <button class="budget-report-btn">Budget Report</button>
</div>
...
document.addEventListener("DOMContentLoaded", function() {
    // Get values from PHP
    const totalExpense = <?php echo json_encode($totalExpense); ?> || 0;
